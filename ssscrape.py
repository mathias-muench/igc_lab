...
class SoaringSpotScraper(requests_cache.CachedSession):
    ...
    def get_pilot_igc_links(self, task_day_url: str) -> Iterator[Tuple[str, str, str]]:
        ...
        for row in soup.select('table tbody tr'):
            ...
            yield igc_url, iso_start, iso_finish

    @staticmethod
    def append_times_to_igc(igc_content: bytes, start_time: str, finish_time: str) -> bytes:
    ...
    def scrape_competition(self, competition_url: str) -> None:
        ...
        for day_url in task_days:
            ...
            for igc_url, start_time, finish_time in igc_data:
                print(f"Downloading: {igc_url}")
                filename, content = self.download_igc_data(igc_url)
                content = self.append_times_to_igc(content, start_time, finish_time)
